...
def get_winner(game_board:list) -> bool:
    for i in range(3):
...
def play_game(gamer:str, name_player_1:str, name_player_2:str, sign_0:str, sign_1:str, board:list) -> str:
    while True:
        if gamer == name_player_1:
            ind_2 = get_number_game('Введите координату X клетки;\n')
...
playing_field = [[' ', ' ', ' '], [' ', ' ', ' '], [' ', ' ', ' ']]
print('Это игра Крестики-Нолики, для того, чтобы сделать ход, введите координаты X и Y')
name_0 = input('Как к Вам обращаться?\n')
name_0 = name_0 + '!'
mark_0, mark_1 = get_mark('Выберите X или O:\n')
name_1 = 'Нафаня'
name_1 = name_1 + '!'
print('Меня зовут Нафаня и я буду Вашим сопериком')
